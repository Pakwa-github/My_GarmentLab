...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        from LearningBaseline.rl_hang.rl_utils.vision_encoder import encoder, sem_model
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        scores = self.sem_encoder(obs).reshape(-1,1)
        max_score = th.max(scores, dim=-1)[0]
        idx = th.argmax(scores, dim=-1)
        sel_point = th.sum(scores * obs[0], dim = 0)
        if self.share_features_extractor:
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor) -> Distribution:
        ...
        if isinstance(self.action_dist, DiagGaussianDistribution):
            log_std = th.tensor([-1,-1,-1]).to(self.log_std.device).to(self.log_std.dtype)
